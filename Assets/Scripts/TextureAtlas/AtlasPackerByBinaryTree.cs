...
namespace Orazum.SpriteAtlas
{
    ...
    // Now focusing on AtlasPackerByFreeLinkedList
    /// <summary>
    /// Not finished work
    /// </summary>
    [System.Obsolete]
    class AtlasPackerByBinaryTree : AtlasPacker
    {
        Node _root;
        ...
        public Sprite Insert(int width, int height)
        {
            ...
            Sprite rectangle = placementNode.GetRectangle();
            UpdateTotalDims(rectangle);

            return rectangle;
        }

        void UpdateTotalDims(Sprite rectangle)
        {
    ...
    class Node
    {
        ...
        {
            _isEmpty = true;
            _rectangle = new Sprite(int2.zero, new int2(Infinite, Infinite));
        }

        public Sprite GetRectangle()
        {
            return _rectangle;
        }
        ...
        Node Split(int2 spriteDims)
        {
            ...
            if (isHorizontalSplit)
            {
                ...
                _left._rectangle = new Sprite(_rectangle.Pos, newDim);
                int2 newPos = new int2(_rectangle.Pos.x, _rectangle.Pos.y + spriteDims.y);
                newDim = new int2(_rectangle.Dims.x, _rectangle.Dims.y);
                _right._rectangle = new Sprite(newPos, newDim);
                Debug.Log($"Horizontal, {_left._rectangle} {_right._rectangle}");

                DebugUtilities.DrawSprite(_left._rectangle, Color.red, 10000);
                DebugUtilities.DrawSprite(_right._rectangle, Color.green, 10000);
            }
            else
            {
        ...
        bool DoesFitRectangle(int2 spriteDims, Sprite rectangle)
        {
        ...
        bool DoesFitPerfectly(int2 spriteDims, Sprite rectangle)
        {
